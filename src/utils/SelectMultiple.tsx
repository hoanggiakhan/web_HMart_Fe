...
interface SelectMultipleProps {
	values: ProductTypeModel[]; // Giá trị để in ra dữ liệu select
...
export const SelectMultiple: React.FC<SelectMultipleProps> = (props) => {
	const handleChange = (
		...
		const dataSelected = props.values.filter((i) =>
			value.includes(i.nameProductType)
		);
		// Từ dữ liệu trên lọc và lấy id ra
		const dataSelectedId = dataSelected.map((i) => i.idProductType);
	...
	return (
		<div>
			<FormControl sx={{ mb: 3, width: "100%" }} size='small'>
				...
				>
					{props.values.map((productType) => (
						<MenuItem key={productType.idProductType} value={productType.nameProductType}>
							<Checkbox
								checked={
									props.selectedListName.indexOf(productType.nameProductType) > -1
								}
							/>
							<ListItemText primary={productType.nameProductType} />
						</MenuItem>
					))}
				</Select>
			</FormControl>
