...
const SelectQuantity: React.FC<SelectQuantityProps> = (props) => {
	...
	const handleQuantity = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (
			...
			// cái isExistsanPham này sẽ tham chiếu đến cái cart ở trên, nên khi update thì cart nó cũng update theo
			let isExistsanPham = cart.find(
				(cartItem) => cartItem.sanPham.maSanPham === props.sanPham?.maSanPham
			);
			// Thêm 1 sản phẩm vào giỏ hàng
			if (isExistsanPham) {
				// nếu có rồi thì sẽ gán là số lượng mới
				isExistsanPham.soLuong = newQuantity;
				...
				if (isToken()) {
					...
					fetch(`http://localhost:8080/gio-hang/update-item`, {
						...
						body: JSON.stringify({
							idCart: isExistsanPham.maGioHang,
							quantity: isExistsanPham.soLuong,
						}),
