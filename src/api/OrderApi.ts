...
export async function get1Orders(idOrder: number): Promise<OrderModel> {
   ...
   await Promise.all(responseOrderDetail._embedded.orderDetails.map(async (orderDetail: any) => {
      const responseProduct = await request(endpointBE + `/order-detail/${orderDetail.idOrderDetail}/book`);
      cartItems.push({ product: responseProduct, quantity: orderDetail.quantity});
   }));
